...
import { groupMods, installedVersion } from "../utils/modGroups";
import { LibraryCleanup } from "../components/LibraryCleanup";
import { LibraryGrouping } from "../components/LibraryGrouping";
...
interface Props {
  onUngroupExisting?: (id: string) => Promise<void>;
  onRestoreComponents?: (id: string) => Promise<void>;
  ...
  /** False until current load-order data was read successfully. */
  loadOrderLoaded?: boolean;
...
function matches(mod: ModSummary, query: string, filter: Filter): boolean {
  ...
  if (filter === "conflicts" && mod.conflictCount === 0 && mod.potentialConflictCount === 0) return false;
...
export function ModsPage({ onUngroupExisting, onRestoreComponents, onBundleAction, onBulkRemove, onClearTemporary, temporaryCount = 0, mods, loadOrder, loadOrderLoaded = true, orderPreview, busy, orderBusy, onInstall, onToggle, onUninstall, onReconfigure, onVerify, onRename, onOpenInstalled, onOpenSource, onBrowseNexus, onPreviewOrder, onApplyOrder, onApplyUe4ssOrder, onCancelOrder, onDiscover, discovering, onOpenModPage, onSetHidden }: Props) {
  ...
  const elsewhere = useMemo(() => {
    ...
    if (runtime > 0) entries.push([`${runtime} UE4SS mod${runtime === 1 ? "" : "s"}`, "UE4SS manages these separately from packaged mod priority."]);
    ...
    if (plugins > 0) entries.push([`${plugins} Unreal plugin mod${plugins === 1 ? "" : "s"}`, "Plugin ordering is not managed here."]);
    return entries;
  }, [mods]);
  ...
  const detail = useMemo(() => selected && (mods.find(mod => mod.id === selected.id) ?? null), [mods, selected]);
  ...
  const library = mods.length === 0 ? <EmptyState title="Your mod library is empty" body="Install a downloaded archive or mod folder to get started." action={<><button className="primary" onClick={onInstall}>Choose a mod</button><button onClick={onBrowseNexus}><ExternalLink aria-hidden size={17} />Browse on Nexus Mods</button></>} /> : <>
    ...
    {visibleGroups.length === 0 ? <EmptyState title={filter === "hidden" ? "No hidden mods match this view" : "No mods match this view"} body="Clear the search or change the filters. Hiding a mod does not change whether it is enabled." action={<button onClick={() => { setQuery(""); setFilter("all"); setHideComponents(false); }}>Reset filters</button>} /> : <section className="mod-list" aria-label="Installed mods">
      <div className="mod-list-head"><span>Status</span><span>Mod</span><span>Type</span><span>Recorded conflicts</span><span>Installed</span><span>Actions</span></div>
      {visibleGroups.map(group => (group.members.length > 1 || !!group.members[0].bundleId) ? <article className="mod-row bundle-row" key={group.id}>
        ...
        <div className="mod-name"><span className="mod-icon"><Package aria-hidden size={19} /></span><div><b title={group.name}>{group.name}</b><small>{installedVersion(group.members)}</small><small>{group.members.length} components</small></div></div>
        <span className="type-chip">Bundle</span>
        {group.members.some(mod => mod.conflictCount || mod.potentialConflictCount) ? <StatusBadge status="warning">{group.members.some(mod => mod.conflictCount) ? "Review active overlaps" : "Potential overlaps"}</StatusBadge> : <span className="muted" title="No overlaps recorded in available package metadata. This does not verify every asset or in-game compatibility.">None recorded</span>}
        ...
        <label className="switch"><input type="checkbox" aria-label={`Enable ${mod.name}`} checked={mod.enabled} disabled={!!busy} onChange={() => onToggle(mod)} /><span /><em>{mod.enabled ? "Enabled" : "Disabled"}</em></label>
        <div className="mod-name"><span className="mod-icon"><Package aria-hidden size={19} /></span><div><b title={mod.name}>{mod.name}</b><small>{installedVersion([mod])}</small></div></div>
        ...
        {mod.conflictCount || mod.potentialConflictCount ? <StatusBadge status="warning">{mod.conflictCount ? `${mod.conflictCount} active` : `${mod.potentialConflictCount} potential`}</StatusBadge> : <span className="muted" title="No overlaps recorded in available package metadata. This does not verify every asset or in-game compatibility.">None recorded</span>}
  ...
  const orderView = !loadOrderLoaded ? <EmptyState title="Load order unavailable" body="Load-order data has not been read successfully. Restart the manager to retry; no ordering changes have been made." /> : loadOrder.entries.length === 0 && loadOrder.ue4ssEntries.length === 0 ? <EmptyState title="No packaged mods to order" body={elsewhere.length > 0 ? `Only IoStore and PAK mods have a deployment order. ${elsewhere.map(([label, reason]) => `${label}: ${reason}`).join(" ")}` : "IoStore and PAK mods appear here after installation. UE4SS mods keep their own runtime ordering."} /> : <div className="order-layout">
    <section className="order-main" aria-label="Packaged mod priority">
      ...
      <div className="order-explainer"><div><h2>Packaged mod priority</h2><p>Higher entries take priority after applying supported ordering. Nothing changes in the game folder until you review and apply.</p></div><span>{activeOverlaps} recorded active overlap{activeOverlaps === 1 ? "" : "s"}</span></div>
      <div className="order-list">
        {orderIds.map((id, index) => { const entry = byId.get(id)!; const wins = loadOrder.activeConflicts.filter(group => winnerFor(group, orderIds, loadOrder.entries) === id).length; const loses = loadOrder.activeConflicts.some(group => group.memberIds.includes(id) && winnerFor(group, orderIds, loadOrder.entries) !== null && winnerFor(group, orderIds, loadOrder.entries) !== id); return <article className={`order-row ${entry.enabled ? "" : "disabled"}`} key={id} onDragOver={event => event.preventDefault()} onDrop={event => { const bounds = event.currentTarget.getBoundingClientRect(); dropOn(id, event.clientY >= bounds.top + bounds.height / 2, event.dataTransfer.getData("text/plain")); }}>
          ...
          <div className="order-health">{wins > 0 ? <span className="winner"><Trophy aria-hidden size={15} />{dirty || loadOrder.unapplied ? "Would win" : "Wins"} {wins}</span> : loses ? <span className="loser">{dirty || loadOrder.unapplied ? "Would lose overlap" : "Loses overlap"}</span> : entry.activeConflictCount ? <span>Overlap recorded</span> : entry.potentialConflictCount ? <span>Potential overlap</span> : <span>No recorded overlap</span>}</div>
      ...
      {dirty && <div className="order-changebar" role="status"><div><b>Load order changed</b><span>Review the exact filenames before applying.</span></div><button onClick={() => { setDraftIds(canonicalIds); onCancelOrder(); }} disabled={orderBusy}>Discard</button><button className="primary" onClick={() => onPreviewOrder(orderIds)} disabled={orderBusy}>Review changes</button></div>}
      {!dirty && loadOrder.unapplied && <details className="unsupported-order"><summary>Optional managed ordering</summary><p>Existing filenames or ranks differ from the manager's naming scheme. This is not an unsaved change or a conflict. Keep them as they are unless you want the manager to apply the displayed priority.</p><button onClick={() => onPreviewOrder(orderIds)} disabled={orderBusy}>Review filename changes</button></details>}
      </>}
      {ue4ssOrder}
    ...
    <aside className="conflict-panel panel"><p className="eyebrow">CONFLICT WINNERS</p><h2>Package overlaps</h2>{conflicts.length === 0 ? <p className="muted">No overlaps in available package metadata. Assets inside unindexed containers are not checked.</p> : conflicts.map(group => { const winner = winnerFor(group, orderIds, loadOrder.entries); const unsupportedMember = group.memberIds.some(id => byId.get(id)?.supported === false); const status = group.active && group.potential ? "Active + potential" : group.active ? "Active" : "Potential"; return <article key={group.id}><span>{status} · {group.packageCount} package{group.packageCount === 1 ? "" : "s"}</span><b>{group.memberIds.map(id => names.get(id) ?? "Unknown mod").join(" ↔ ")}</b><small>{group.active && winner ? `${names.get(winner)} ${dirty || loadOrder.unapplied ? "would win after applying" : "wins in the managed order"}` : unsupportedMember ? "Winner unavailable for an unsupported layout" : "No verified winner in the available ordering data"}</small></article>; })}</aside>
  </div>;
  ...
  return <div className="page">
    ...
    {onUngroupExisting && onRestoreComponents && <LibraryGrouping mods={mods} busy={!!busy || orderBusy || !!discovering} onUngroup={onUngroupExisting} onRestore={onRestoreComponents} />}
    ...
    {detail && <section className="panel mod-details" aria-label={`${detail.name} details`}><button className="detail-close" onClick={() => setSelected(null)} aria-label="Close details"><X size={17} /></button><p className="eyebrow">MOD DETAILS</p><h2>{detail.name}</h2><div className="detail-list"><div><span>Version</span><b>{detail.version ?? "Not provided"}</b></div><div><span>Type</span><b>{detail.modType}</b></div><div><span>Status</span><b>{detail.enabled ? "Enabled" : "Disabled"}</b></div><div><span>Game build when installed</span><b>{detail.installedBuild ?? "Unknown"}</b></div><div><span>Indexed packages</span><b>{detail.packageCount || "Not indexed"}</b></div><div><span>Recorded active overlaps</span><b>{detail.conflictCount}</b></div>{detail.bundleId && <div><span>Install group</span><b>{mods.filter(mod => mod.bundleId === detail.bundleId).length} components in this package</b></div>}</div>
