...
export const backend = {
  ...
  ungroupExistingMods: (id: string) => invoke<number>("ungroup_existing_mods", { id }),
  factoryResetPreview: () => invoke<{ directoriesToReset: string[]; recoveryDirectories: string[]; externalLibraryRetained: string | null }>("factory_reset_preview"),
  resetApplication: () => invoke<void>("reset_application", { confirmation: "RESET" }),
  restoreAdoptedComponents: (id: string) => invoke<number>("restore_adopted_components", { id }),
...
const changedFilePrefix = "A managed file changed outside Zero Mod Manager: ";
const rolledBackPrefix = "No package changes were kept. Previous files and library were restored. ";
const oldExplanation = ". Some mods write their own settings or data files there while the game runs.";

/** Recover the guarded path from both direct failures and successful rollbacks. */
export function changedManagedFile(error: unknown): string | null {
  const message = friendlyError(error);
  const direct = message.startsWith(changedFilePrefix);
  const rolledBack = message.startsWith(rolledBackPrefix + changedFilePrefix);
  if (!direct && !rolledBack) return null;
  let path = message.slice((rolledBack ? rolledBackPrefix.length : 0) + changedFilePrefix.length);
  if (path.endsWith(oldExplanation)) path = path.slice(0, -oldExplanation.length);
  return path.trim() || null;
}

export function isChangedFileError(error: unknown): boolean {
  return changedManagedFile(error) !== null;
}
