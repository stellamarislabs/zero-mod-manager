import { ArchiveRestore, KeyRound, Loader2, X } from "lucide-react";
...
export function LegacyImportDialog({ status, busy, onImport, onClose }: {
  ...
  busy: boolean;
  onImport: (includeNexusKey: boolean) => void;
  onClose: () => void;
}) {
  ...
  return <div className="dialog-backdrop" role="presentation">
    <section className="panel legacy-import-dialog" role="dialog" aria-modal="true" aria-labelledby="legacy-import-title">
      ...
      <label className="legacy-key-option"><input type="checkbox" checked={includeKey} onChange={event => setIncludeKey(event.target.checked)} disabled={busy} /><KeyRound aria-hidden size={17} /><span><b>Also import my Nexus API key</b><small>Optional. This reads the old app’s saved credential and stores a separate copy for Zero Mod Manager.</small></span></label>
      <small className="legacy-source">Source: {status.dataDirectory}</small>
      <div className="dialog-actions"><button disabled={busy} onClick={onClose}>Not now</button><button className="primary" disabled={busy} onClick={() => onImport(includeKey)}>{busy ? <Loader2 className="spin" aria-hidden size={17} /> : <ArchiveRestore aria-hidden size={17} />}{busy ? "Importing and verifying…" : "Import library"}</button></div>
    </section>
  </div>;
}
