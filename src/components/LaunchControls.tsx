...
export function LaunchControls({ status, canLaunch, launching, onHealth, onLaunchGame, onLaunchMode, onProfiles }: {
  ...
  return <section className="command-bar" aria-label="Launch and readiness controls">
    <button className={`command-readiness ${status}`} onClick={onHealth} title="Readiness reflects available local checks, not in-game compatibility." aria-label={`Readiness: ${label}. Open Health`}><span className={`op-state ${status}`}>{label}</span></button>
    ...
    <button onClick={() => onLaunchMode ? onLaunchMode("vanilla") : onLaunchGame()} disabled={!canLaunch || launching || status === "blocked"} title="Temporarily disables manager-owned mods. Runtime and unmanaged files remain."><Play aria-hidden size={17} />Launch vanilla</button>
    <button onClick={() => setConfirmTroubleshoot(true)} disabled={!onLaunchMode || !canLaunch || launching || status === "blocked"}><Wrench aria-hidden size={17} />Troubleshoot</button>
