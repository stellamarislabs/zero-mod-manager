...
if ($LASTEXITCODE -ne 0) { throw 'Application branding verification failed' }
...
try {
    ...
    & "$PSScriptRoot/verify-windows-icons.ps1" -Executable $exe -Installer (Join-Path $root "src-tauri/target/release/bundle/nsis/Zero Mod Manager_${version}_x64-setup.exe")
