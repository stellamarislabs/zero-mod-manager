...
/// Reset may only be scheduled while no package transaction can be interrupted.
pub fn while_idle<T>(action: impl FnOnce() -> Result<T>) -> Result<T> {
    let _guard = OPERATION.try_lock().map_err(|_| {
        error("Wait for the current mod operation to finish before resetting the app.")
    })?;
    action()
}
struct ResetActive;
...
impl Journal {
    ...
    fn safe_tree(&self, path: &Path) -> Result<()> {
        ...
        if path.parent() != Some(self.library.as_path())
            || uuid::Uuid::parse_str(
                name.strip_prefix(".replacing-")
                    .or_else(|| name.strip_prefix(".adopting-"))
                    .unwrap_or(name),
            )
            .is_err()
        {
