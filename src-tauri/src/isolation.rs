...
fn instruction(
    ...
    if status == "completed" {
        return match phase {
            "baseline-failed" => "The vanilla baseline also failed. The installed mod set cannot be isolated until the game or launcher baseline works.".into(),
            "runtime-suspect" => "Vanilla worked but runtime-only failed. Review UE4SS/proxy/VC runtime evidence before blaming a content mod.".into(),
    ...
    match phase {
        "baseline" => "Launch a vanilla baseline. Use the main menu or a disposable test save; save files are not managed.".into(),
        "runtime" => "Launch runtime-only with all manager mods withdrawn. This separates loader health from content mods.".into(),
        "bisect" => format!("Launch the next dependency-safe subset ({} mods), then label what happened.", current.len()),
