...
pub fn retire(conn: &Connection) -> Result<()> {
    ...
    for key in ["nexus_api_key", "nexus_account_name", "nexus_premium", "nexus_auto_update_check"] {
    ...
    match entry.delete_credential() {
        ...
        Err(error) => return Err(AppError::Other(format!("Retired credential cleanup: {error}"))),
    }
