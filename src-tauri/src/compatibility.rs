...
pub fn report(conn: &Connection) -> Result<CompatibilityReport> {
    report_for_build(conn, None)
}

/// The current detection result is supplied by the caller. A cached build from
/// an earlier installation/startup must never be called the current game build.
pub fn report_for_build(
    conn: &Connection,
    ...
    for item in enabled.values() {
        ...
        if let (Some(game), Some(build)) = (&manifest.game, current_build) {
